...
def financial_summary(symbol):
    ...
    try:
        ...
        income_filtered = (
            data["income"][data["income"]["report_type"] == 1]
            if not data["income"].empty
            else pd.DataFrame()
        )
        balance_filtered = (
            data["balancesheet"][data["balancesheet"]["report_type"] == 1]
            if not data["balancesheet"].empty
            else pd.DataFrame()
        )
        cashflow_filtered = (
            data["cashflow"][data["cashflow"]["report_type"] == 1]
        ...
        if not indicator_df.empty and "report_type" in indicator_df.columns:
            indicator_filtered = indicator_df[
                indicator_df["report_type"].astype(str) == "1"
            ]
            indicator = (
                indicator_filtered.iloc[0] if not indicator_filtered.empty else None
            )
        else:
            indicator = None
        ...
        # 利润指标
        if income is not None:
            summary["total_operate_revenue"] = income.get(
                "total_revenue"
        ...
        if balance is not None:
            ...
            summary["total_hldr_eqy_exc_min_int"] = balance.get(
        ...
        if indicator is not None:
            ...
            summary["netprofit_margin"] = indicator.get(
                "netprofit_margin"
            )  # 销售净利率
            summary["grossprofit_margin"] = indicator.get(
                "grossprofit_margin"
            )  # 销售毛利率
            summary["operateprofit_margin"] = indicator.get(
            ...
            summary["netprofit_yoy"] = indicator.get(
                "netprofit_yoy"
            )  # 净利润同比增长率
            ...
            summary["debt_to_eqt"] = indicator.get(
                "debt_to_eqt"
            )  # 权益乘数(负债权益比)
        ...
        try:
            ...
            with fq.engine.connect() as conn:
                valuation_df = pd.read_sql_query(
                    valuation_query, conn, params={"symbol": f"{code}%"}
                )
            ...
            logging.getLogger(__name__).warning(
                f"Failed to get valuation for {symbol}: {e}"
            )
        ...
        for name, (field, unit) in indicator_map.items():
            ...
            if value is not None:
                ...
                if unit == "元" or unit == "万元":
                    ...
                    display_value = (
                        f"{value * 100:.2f}%" if abs(value) < 1 else f"{value:.2f}%"
                    )
                else:
                    display_value = (
...
def _get_valuation_trend(symbol: str) -> list:
    """
    获取估值趋势数据（PE/PB/PS/市值）
    从 DuckDB bars_a_1d 表获取最近8个季度末的估值数据
    """
    import duckdb
    ...
    try:
        ...
        quarter_ends.reverse()  # 按时间升序

        # 获取每个季度末最近的估值数据
        valuation_data = []
        for qe in quarter_ends:
            query = """
            SELECT datetime, pe, pe_ttm, pb, ps, ps_ttm, total_mv / 10000.0 as total_mv_yi
            FROM bars_a_1d
            WHERE stock_code = ?
            AND datetime <= ?
            AND pe IS NOT NULL
            ...
            LIMIT 1
            """
            result = conn.execute(query, [symbol, qe]).fetchone()
            if result:
                date_str = result[0].strftime('%Y%m%d') if result[0] else None
                if date_str:
                    # PE
        ...
        import logging
        logging.getLogger(__name__).warning(f"Failed to get valuation trend for {symbol}: {e}")
        return []
...
def financial_report(symbol):
    ...
    try:
        ...
        CASHFLOW_FIELDS = {
            ...
            "投资活动现金流量小计": "n_cashflow_inv_act",
            "筹资活动现金流量小计": "n_cash_flows_fnc_act",
            "现金及现金等价物增加额": "n_cash_incr",
            "期末现金及现金等价物余额": "end_bal_cash",
        }
        ...
        result = {"income": [], "balance": [], "cashflow": [], "indicators": []}
        ...
        if not data.get("balancesheet", pd.DataFrame()).empty:
            df_sorted = (
                data["balancesheet"].sort_values("end_date", ascending=False).head(8)
            )
            for _, row in df_sorted.iterrows():
        ...
        if not data["cashflow"].empty:
            df_sorted = (
                data["cashflow"].sort_values("end_date", ascending=False).head(8)
            )
            for _, row in df_sorted.iterrows():
        ...
        if "fina_indicator" in data and not data["fina_indicator"].empty:
            df_sorted = (
                data["fina_indicator"].sort_values("end_date", ascending=False).head(8)
            )
            field_names = {
                "roe": "净资产收益率",
                "roa": "总资产报酬率",
        ...
        # 获取估值趋势数据（PE/PB/PS/市值）
        result["valuation"] = _get_valuation_trend(symbol)
...
def financial_full(symbol):
    ...
    try:
        ...
        return {"success": True, "symbol": symbol, "data": result}, 200
    except Exception as e:
        import traceback
...
def financial_valuation(symbol):
    ...
    try:
        ...
        return {"success": True, "symbol": symbol, "valuation": result}, 200
    except Exception as e:
        import traceback
...
def get_available_periods(symbol):
    """
    获取股票的可用财务报告期列表（纯数据质量检查）
    ...
    try:
        ...
        from src.valuation.data.available_periods_provider import (
            AvailablePeriodsProvider,
        )

        # 获取查询参数
        required_fields_str = request.args.get("required_fields", "")
        date = request.args.get("date")
        limit = int(request.args.get("limit", 20))

        # 解析required_fields参数
        provider = AvailablePeriodsProvider("data/tushare_data.db")

        if required_fields_str:
            required_fields = (
                AvailablePeriodsProvider.parse_required_fields_from_string(
                    required_fields_str
                )
            )
        else:
        ...
        # 获取可用报告期
        result = provider.get_available_periods(

...
import { ValuationSearch, ValuationParamsForm, ValuationResultCard, ModelComparisonChart, DcfConfigForm } from '@/components/valuation';
...
function ValuationPage() {
  ...
  const [methods, setMethods] = useState<ValuationMethod[]>(['combined']);
  const [date, setDate] = useState('');
  const [combineMethod, setCombineMethod] = useState<CombineMethod>('weighted');
  const [dcfConfig, setDcfConfig] = useState<DCFConfig>({});  // 默认为空，不自动启用DCF
  ...
  const handleReset = () => {
    setStockCode('');
    setDcfConfig({ risk_profile: 'balanced' });
    clear();
  };
